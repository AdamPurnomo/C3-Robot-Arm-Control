...
void Jacob_dot (double mat1[7][3], double mat2[3][1], double res[7][1])
{
    int i, j, k;
    for (i = 0; i < 7; i++)
    {
...
{
    int i;
    double J[3][7], Jpseudo[7][3], cv[3][1], jv[7][1];
    for(i=0;i<3;i++) cv[i][0] = carvel[i];
    Jac_calc(J,theta);
    PseudoJacobian(J,Jpseudo);
    Jacob_dot(Jpseudo,cv,jv);
    for(i=0;i<7;i++) jointvel[i] = jv[i][0];
}

int main()
{
    int i,j;
    double J[3][7], theta[7];
    double Jps[7][3];
    double vel[3][1], velo[3];
    vel[0][0] = 0;
    vel[1][0] = -24.742757;
    vel[2][0] = -11.160556;
    velo[0] = 0;
    ...
    ResolvedMotionRate(omegao,velo,theta);
    for(i=0;i<7;i++) printf("%lf ", omegao[i]*cons);
    printf("\n");

    Jac_calc(J, theta);
    PseudoJacobian(J, Jps);
    Jacob_dot(Jps,vel,omega);
    for(i=0;i<7;i++) printf("%lf ", omega[i][0]*cons);
    printf("\n");
