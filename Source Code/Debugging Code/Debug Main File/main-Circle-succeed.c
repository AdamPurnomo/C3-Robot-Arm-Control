...
const double mass[7] = {5.87, 4.71, 4.65, 1.97, 1.62, 1.01, 0.073};
const double length[7] = {0.1, 0.25, 0.15, 0.063, 0.287, 0.06, 0.005};
const double com[7] = {0.048, 0.14, 0.075, 0.02, 0.175, 0.02, 0.0025};
...
circleCtrl(struct params *param, int trig)
{
	...
	pdCtrl(des_j, cur_j);
	 if(tick%1000==0)
    {
        printf("%lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf\n", tick*TICKS, RAD2DEG*des_j[0].pos, RAD2DEG*des_j[1].pos, RAD2DEG*des_j[2].pos, RAD2DEG*des_j[3].pos, RAD2DEG*des_j[4].pos, RAD2DEG*des_j[5].pos, RAD2DEG*des_j[6].pos, RAD2DEG*cur_j[0].pos, RAD2DEG*cur_j[1].pos, RAD2DEG*cur_j[2].pos, RAD2DEG*cur_j[3].pos, RAD2DEG*cur_j[4].pos, RAD2DEG*cur_j[5].pos, RAD2DEG*cur_j[6].pos, torque[0], torque [1], torque[2], torque[3], torque[4], torque[5], torque[6], cpath.T[0][3], cpath.T[1][3], cpath.T[2][3], RAD2DEG*cpath.phase);
    }
    tick++;
...
linepathGenerate(struct status *Des_j, unsigned long Time)
{
	...
	FK(cur_th, T);
       /*if(cur_j[1].pos <0 && cur_j[3].pos<0 && cur_j[5].pos<0) IKneg(th, lpath.T);
    else if(cur_j[1].pos >0 && cur_j[3].pos>0 && cur_j[5].pos>0) IKpos(th, lpath.T);
    else IKpn(th, lpath.T);*/
	...
	if (Time% 1000 == 0)
	{
		/*printf("%lf, %lf, %lf, %lf, %lf, %lf, %lf\n", RAD2DEG * cur_th[0], RAD2DEG * cur_th[1], RAD2DEG * cur_th[2], RAD2DEG * cur_th[3], RAD2DEG * cur_th[4], RAD2DEG * cur_th[5], RAD2DEG * cur_th[6]);*/
...
circlepathGenerate(struct status *Des_j, unsigned long Time)
{
    ...
    /*if(cur_j[1].pos <0 && cur_j[3].pos<0 && cur_j[5].pos<0) IKneg(th, cpath.T);
    else if(cur_j[1].pos >0 && cur_j[3].pos>0 && cur_j[5].pos>0) IKpos(th, cpath.T);
    else IKpn(th, cpath.T);*/
...
for(i=0; i<7; i++)
{
    ...
    torque[i] = inertia[i] * accel[i] - 0.6*tg[i];
...
{
    double destime = 10;
    double theta[7] = {-10*DEG2RAD, -15*DEG2RAD, 0*DEG2RAD, -60*DEG2RAD, -12*DEG2RAD, -20*DEG2RAD, -10*DEG2RAD};
