...
void Tranf_Matrix(double arr[4][4],int alpha, int a, int d, float theta)
{
...
void InverseKin1 (double F[4][4], double theta[7])
{
    ...
    double hx = cos(t1*val)*gx + sin(t1*val)*gy - 100;
    double hy = gz-320;

    double k = (hx*hx + hy*hy - 350*350 - 400*400)/(2*350*400);
    t4 = 2*cons*atan2(sqrt(1-k), sqrt(1+k));
    /*printf("%lf, %lf, %lf\n", hx, gx, k);*/

    double k1 = 350*cos(t4*val)+400;
    double k2 = 350*sin(t4*val);
    double p1 = -k2 + sqrt(k2*k2 + k1*k1 - hy*hy);
    double p2 = hy+k1;
    t2 = 2*cons*atan2(p1, p2);
    /*printf("tan(t2/2) = %lf\n", (p1/p2));*/
    theta[0]=t1;
    theta[1]=t2;
    theta[2]=t3;
...
void IK (double theta[7], double T[4][4])
{
