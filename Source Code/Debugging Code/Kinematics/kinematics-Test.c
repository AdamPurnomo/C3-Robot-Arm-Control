...
void Tranf_Matrix(double arr[4][4],double alpha, double a, double d, double theta)
{
    arr[0][0] = cos(theta);
...
{
    double t1=0, theta1[2] ;
    double t2=0, theta2[2] ;
    double t3 = 0;
    double t4=0 , theta4[4];
    int i;
    ...
    theta1[0] = atan2(-gy, -gx);
    theta1[1] = atan2(gy, gx);
    min = 100.;
    for(i=0;i<2;i++)
    {
        diff[i] = fabs(theta1[i] - pt[0]);
        if(diff[i]<min)
        {
            min = diff[i];
            t1 = theta1[i];
        }
        /*printf("(%lf, %lf), (%lf, %lf)\n", theta1[i], pt[0], diff[i], min);*/
    }
    ...
    theta4[0] = 2*atan2(sqrt(1-k), sqrt(1+k));
    theta4[1] = 2*atan2(-sqrt(1-k), -sqrt(1+k));
    theta4[2] = 2*atan2(-sqrt(1-k), sqrt(1+k));
    theta4[3] = 2*atan2(sqrt(1-k), -sqrt(1+k));
    min = 100.;
    for(i=0;i<4;i++)
    {
        diff[i] = fabs(theta4[i] - pt[3]);
        if(diff[i]<min)
        {
            min = diff[i];
            t4 = theta4[i];
    ...
    double p2 = hy+k1;
    double q1 = -k2 - sqrt(k2*k2 + k1*k1 - hy*hy);
    theta2[0] = 2*atan2(p1, p2);
    theta2[1] = 2*atan2(q1, p2);
    min = 100.;
    for(i=0;i<2;i++)
    {
        diff[i] = fabs(theta2[i] - pt[1]);
        if(diff[i]<min)
        {
            min = diff[i];
            t2 = theta2[i];
...
{
    double theta5[2], t5=0;
    double theta6[2], t6=0;
    double t7=0;
	int i, j, k;
	double diff[4]={0., 0., 0., 0.};
    double min;
    ...
    multiply(Jdummy,F,J);
    theta5[0] = atan2(-J[1][2], -J[0][2]);
    theta5[1] = atan2(J[1][2], J[0][2]);
    t5 = atan2(-J[1][2], -J[0][2]);
    min = 100.;
    for(i=0;i<2;i++)
    {
        diff[i] = fabs(theta5[i] - pt[4]);
        if(diff[i]<min)
        {
            min = diff[i];
            t5 = theta5[i];
    ...
    theta6[0] = atan2(-J[0][2]*cos(t5) - J[1][2]*sin(t5), J[2][2]);
    theta6[1] = atan2(J[0][2]*cos(t5) + J[1][2]*sin(t5), -J[2][2]);
    min = 100.;
    for(i=0;i<2;i++)
    {
        diff[i] = fabs(theta6[i] - pt[5]);
        if(diff[i]<min)
        {
            min = diff[i];
            t6 = theta6[i];
    ...
    /*theta7[7] = atan2(J[0][0]*sin(t5) - J[1][0]*cos(t5) , J[0][1]*sin(t5) - J[1][1]*cos(t5) );
    min = 100.;
   for(i=0;i<2;i++)
    {
        diff[i] = fabs(theta7[i] - pt[6]);
        if(diff[i]<min)
        {
            min = diff[i];
            t7 = theta7[i];
...
void IK (double theta[7], double T[4][4], double pt[7])
{
    ...
    InverseKin1(T,theta,pt); /*calculate the first three angke*/
