...
extern void jointmode1(void);
extern void jointmode2(void);
extern void jointmode3(void);
extern void linemode (void);
extern void RMRlinemode(void);
extern void RMRcirclemode(void);
...
//    if(debug) for(j=0;j<nbytes;j++) printf("cpc_msg[%2d] = %d\n",j,(unsigned char)buffer[j]);
	...
	if (!(strncmp(cpc_com, "RMRlinemode", 11))) { RMRlinemode(); return; };
	if (!(strncmp(cpc_com, "circlemode", 10))) { circlemode(); return; };
	if (!(strncmp(cpc_com, "RMRcirclemode", 13))) { RMRcirclemode(); return; };
	if (!(strncmp(cpc_com, "jointmode1", 10))) { jointmode1(); return; };
	if (!(strncmp(cpc_com, "jointmode2", 10))) { jointmode2(); return; };
	if (!(strncmp(cpc_com, "jointmode3", 10))) { jointmode3(); return; };
